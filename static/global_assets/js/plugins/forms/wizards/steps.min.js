...
!function (a, b) {
    ...
    function g(b, c) {
        ...
        var e = b.find(".trainer_content > *");
        e.removeData("loaded").removeData("mode").removeData("url"), e.removeAttr("id").removeAttr("role").removeAttr("tabindex").removeAttr("class").removeAttr("style")._removeAria("labelledby")._removeAria("hidden"), b.find(".trainer_content > [data-mode='async'],.trainer_content > [data-mode='iframe']").empty();
        var f = a('<{0} class="{1}"></{0}>'.format(b.get(0).tagName, b.attr("class"))), g = b._id();
        return null != g && "" !== g && f._id(g), f.html(b.find(".trainer_content").html()), b.after(f), b.remove(), f
    }
    ...
    function x(b, c, d, e, f) {
        ...
        var g = b.find(".trainer_content"), h = a("<{0}>{1}</{0}>".format(c.headerTag, f.title)),
    ...
    function A(b, c, d) {
        if (d.stepCount > 0) {
            ...
            if (!c.enableContentCache || !f.contentLoaded) switch (r($, f.contentMode)) {
                case $.iframe:
                    b.find(".trainer_content > .body").eq(d.currentIndex).empty().html('<iframe src="' + f.contentUrl + '" frameborder="0" scrolling="no" />').data("loaded", "1");
                    break;
                case $.async:
    ...
    function E(b, c, d, e) {
        var f = j(b, d.currentIndex), g = a('<span class="current-info audible">' + c.labels.current + " </span>"),
            h = b.find(".trainer_content > .title");
        if (null != e) {
            var i = j(b, e);
    ...
    function J(b, c, d) {
        var e = '<{0} class="{1}">{2}</{0}>', f = r(_, c.stepsOrientation), g = f === _.vertical ? " vertical" : "",
            h = a(e.format(c.contentContainerTag, "trainer_content " + c.clearFixCssClass, b.html())),
    ...
    function P(b, c, d, e, f, g) {
        var h = b.find(".trainer_content > .body"), i = r(ab, c.transitionEffect), j = c.transitionEffectSpeed,
            k = h.eq(e), l = h.eq(f);
        switch (i) {
            case ab.fade:
            case ab.slide:
